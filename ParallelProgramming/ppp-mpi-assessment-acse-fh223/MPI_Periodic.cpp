...
void grid_to_file(int out, int rank, vector<vector<double> > &C1, vector<vector<double> > &C2, int local_imax, int local_jmax)
{
    ...
    fstream f1, f2;
    fname1 << "./MPI_out_periodic/output_C1_" << rank << "_" << out << ".dat";
    f1.open(fname1.str().c_str(), ios_base::out);
    fname2 << "./MPI_out_periodic/output_C2_" << rank << "_" << out << ".dat";
...
// rules for a single time step (Periodic boundary conditions)
void do_iteration_periodic(vector<vector<double> > &C1, vector<vector<double> > &C1_old, vector<vector<double> > &C2, vector<vector<double> > &C2_old, int rank, int size, int local_imax, int local_jmax, MPI_Datatype mpi_custom_type)
{
    ...
    if (rank == 0) {
        for (int j = 0; j < local_jmax; j++) {
            C1[0][j] = C1[local_imax - 2][j];
            C2[0][j] = C2[local_imax - 2][j];
        }
    }
    if (rank == size - 1) {
        for (int j = 0; j < local_jmax; j++) {
            C1[local_imax - 1][j] = C1[1][j];
            C2[local_imax - 1][j] = C2[1][j];
        }
    }

    for (int i = 1; i < local_imax - 1; i++) {
        C1[i][0] = C1[i][local_jmax - 2];
        C1[i][local_jmax - 1] = C1[i][1];
        C2[i][0] = C2[i][local_jmax - 2];
        C2[i][local_jmax - 1] = C2[i][1];
    }
...
int main(int argc, char** argv)
{
    ...
    {
        // Perform iteration
        do_iteration_periodic(C1, C1_old, C2, C2_old, rank, size, local_imax, local_jmax, mpi_custom_type);
